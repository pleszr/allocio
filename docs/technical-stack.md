...
- A deterministic code map (`tools/code_map.py`, Python standard library only) generates the committed `docs/code-map.json`. For each PR, CI generates a self-contained interactive architecture review from the PR range, publishes it through the repository's `gh-pages` branch, and posts only its link to the PR. The page provides a per-area columnar module graph with hover tracing, layer filtering, exact-commit click-through, and a PR-changes-only mode; generated HTML is not duplicated in feature branches.
