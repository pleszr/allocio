...
- every `expense_event` carries an `excluded_from_average` flag (default `false`), settable when the expense is logged and correctable afterward via the same admin-repair exception above. It removes that expense from the dashboard's forward-looking trailing-average KPIs only — average monthly cost, average paid-out-of-pocket, and the recommended manual-extra buffer (`docs/vehicle-rules.md`, "Vehicle Dashboard Signals") — and never affects the bucket balance, check-in totals, or the History ledger, which always reflect every posted expense in full
