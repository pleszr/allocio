...
- If the plan originates from a GitHub issue number, the PR body must include a `Closes #<issue-number>` line (GitHub's auto-close syntax) so merging the PR closes the issue automatically. Place it near the top of the PR body, outside the `Requirements` section.
