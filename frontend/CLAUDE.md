...
- `e2e/` — Playwright browser end-to-end tests (`*.spec.ts`) plus `global-setup.ts`/`global-teardown.ts` and the shared throwaway-DB identity in `db.ts`; config is `playwright.config.ts`
...
- The suite requires the always-on local Postgres container (`docker compose up -d`); it creates and drops a throwaway `allocio_e2e` database per run and never touches the `allocio` dev database. If Postgres is down, global setup fails with the command to start it.
