...
- `src/components/` — presentational + chrome components (Icon, Sparkline, CostDistributionChart, TimeCostPanel, CostHistoryModal, SpatialMap, MaintenancePanel, Illustrations, Sidebar, TopBar, UserMenu, Tabs, StateView). `SpatialMap` exports a shared `CarDiagram` (the car x-ray + region overlay, plus the hover/selection talk-box, driven by an external `focusItem`), the card component `MaintenanceCard`, the `MaintenanceFilterTabs` component (All/Good/Attention/Overdue, with an optional `allLabel` override for the first tab's label), and the `sortByUrgency`/`matchesMaintenanceFilter`/`maintenanceIcon` helpers, all reused by the dashboard's `MaintenancePanel`, which passes `allLabel` to rename its first tab to "Top 5" and caps that tab's list to the 5 most urgent items (the other three tabs and the Costs screen's own `SpatialMap` usage stay uncapped); `CostHistoryModal` exports both `timeCostHistoryTarget` and `maintHistoryTarget` history-target builders so every entry point opens the same popup. `TimeCostPanel` renders an asset's active time-based costs sorted due-soonest-first, alongside a compact 12-month horizon strip (icon markers positioned by days-until-due, colored by urgency, staggered onto a second row when two due dates land within 4% of each other on the strip) and an always-visible this-month/this-year total plus a share-of-annual-total bar and legend; its `timeCostIcon` helper maps a cost's `technical_key` to a glyph (mirroring `SpatialMap`'s `maintenanceIcon` pattern, with a generic-receipt fallback), covering every built-in time-based cost template across vehicle, house, and pet — extend that map when a new template technical_key is added. `EditFormControls` exports the shared `LabeledMoney`/`EditActions` form primitives used across every screen's inline edit rows. `ManualExtraEditor` is the free-form manual-extra editor (a single amount field + Save/Cancel), rendered as a small popover (portaled to `<body>`, positioned off a `getBoundingClientRect()` of its trigger, dismissed on outside click/Escape) anchored to a small inline text-link trigger (Hungarian "Beállítom" / English "Set it") placed right after the "Javasolt extra" recommendation line, or after the plain description when there's no recommendation, on the Dashboard's "Extra kiegészítés" tile — manual extra is edited only from the Dashboard, not from the Costs screen. When there's a backend-computed recommendation (`manual_extra_recommended > 0`), the field prefills with `manual_extra_recommended` directly — that value is already the full target `manual_extra_monthly` should reach (real spend minus `average_allocation`, which itself excludes manual extra), not an incremental top-up on the current value — so accepting it is just hitting Save, no separate quick-apply control.
- `src/screens/` — one file per screen (Home, Dashboard, Costs, CheckIn, History, NewBucket, SignIn)
- `src/utils/` — pure helpers: `format.ts` (currency-agnostic number/date helpers), `assetType.ts`, `health.ts`, and the `useAsync` fetch hook, plus `currency.tsx` — the `CurrencyProvider` React context and `useCurrency()` hook that own money rendering (the display currency symbol, its placement, and decimal precision — HUF renders with 0 decimals, EUR/USD with 2, decided by currency rather than the call site). Money must be rendered through `useCurrency()`'s `fmt`, not a hardcoded symbol. `fmt`'s `decimals` override exists only for per-unit rates (e.g. HUF/km), not currency amounts. Do NOT name this `lib/` — the repo `.gitignore` ignores `lib/`. `useMutation.ts` is the shared busy/error async-mutation-run hook used by every screen's inline edit rows. `manualExtraRecommendation.ts` holds the `localStorage`-backed "already applied" marker helpers behind `ManualExtraEditor`'s idempotency guard, keyed by both the recommended amount and the current-plus-recommended target so a later downward edit un-hides a fulfilled recommendation.
